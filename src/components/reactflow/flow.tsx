...
function Flow() {
  ...
  const onDrop = useCallback(
    (event: any) => {
      ...
      if (reactFlowWrapper.current && reactFlowInstance) {
        ...
        const abiItemString = event.dataTransfer.getData(
  ...
  const handleCustomNodeInputChange = useCallback(
    (nodeId: any, inputIndex: any, value: any) => {
  ...
  const nodeTypes = useMemo(
    () => ({
      abiNode: (props: any) => (
        <CustomNode {...props} onInputChange={handleCustomNodeInputChange} />
      ),
      startNode: StartNode,
    }),
